...
# =============================================================================
# FCIS PRIVATE BRIEFING v2
# VERIFIED CORRELATION INTELLIGENCE
# =============================================================================

def render_correlation_briefing(db):
    """
    Render private human-review intelligence from verified FCIS correlations.
    ...
    if count == 0:
        lines.append("FCIS FOUND NOTHING NEW REQUIRING HUMAN REVIEW")
        lines.append("")
        lines.append(
            "No verified need ↔ offer correlations are currently present."
        )
        lines.append("")
        lines.append("-" * 78)
        lines.append(
    ...
    for number, correlation in enumerate(
        correlations,
        1,
    ):
        ...
        lines.append(
            ...
            f"{need_subject['name']} "
            f"↔ "
            f"{offer_subject['name']}"
        )

        lines.append(
            f"   Signal: {display_key}"
        )

        lines.append("")

        lines.append("   WHY THIS SURFACED")

        lines.append(
            f"   {need_subject['name']} "
            f"has an active need for "
            f"{display_key}."
        )

        lines.append(
            f"   {offer_subject['name']} "
            f"has an active offer for "
            f"{display_key}."
        )
        ...
        if relationships:
            ...
            for relationship in relationships:
                ...
                if (
                    ...
                    lines.append(
                        "   CAUTION: "
                        "This relationship is not active."
                    )
            ...
            lines.append(
                ...
                "currently connects these subjects."
            )
        ...
        lines.append(
            "   CANONICAL FACT CREATED: NO"
        )
        lines.append(
            "   RELATIONSHIP CREATED: NO"
        )
        lines.append(
