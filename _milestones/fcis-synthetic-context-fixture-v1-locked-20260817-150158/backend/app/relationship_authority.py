...
SUPPORTED_ENDPOINT_TYPE = "organization"
...
def _validate_endpoint(
    ...
    if endpoint_type != SUPPORTED_ENDPOINT_TYPE:
        raise ValueError(
            f"{endpoint_role}_type {endpoint_type!r} is not supported "
            "by Canonical Relationship Authority v1."
        )
...
def create_canonical_relationship(
    ...
    *,
    ...
    - both endpoints must have established canonical provenance
    - no self-edge
    ...
    if (
        ...
        raise ValueError(
            "Canonical Relationship Authority v1 does not permit self-edges."
        )
