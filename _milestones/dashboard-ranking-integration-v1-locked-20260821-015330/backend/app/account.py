...
from backend.app.network_pulse_count_aggregation import build_network_pulse_counts
from backend.app.dashboard_ranking_consumer import get_dashboard_ranking_context
...
def account_creator_dashboard(
    ...
    ranking_context = get_dashboard_ranking_context(
        db,
        user_id=user.id,
        organization_ids=[
            record["organization"]["id"]
            for record in active_organizations
        ],
    )

    creator_ranking = ranking_context["creator"]
    ...
    return {
        ...
        "summary": {
            ...
            "leaderboard": {
                "score": creator_ranking["score"] if creator_ranking else None,
                "rank_scope": "creator-platform",
                "rank": creator_ranking["rank"] if creator_ranking else None,
                "status": "scoring_active",
                "note": "Score and rank are supplied by canonical Platform Ranking Aggregation."
            }
        },
        "leaderboard": {
            "score": creator_ranking["score"] if creator_ranking else None,
            "rank": creator_ranking["rank"] if creator_ranking else None,
            "components": creator_ranking["components"] if creator_ranking else {},
            "formula": {
                "authority": "canonical-platform-ranking-aggregation"
            }
        },
        "opportunities": {
            "available": [
...
def account_rescue_dashboard(
    ...
    ranking_context = get_dashboard_ranking_context(
        ...
        organization_ids=[
            record["organization"]["id"]
            for record in rescue_organizations
        ],
    )

    rescue_ranking = ranking_context["rescue"]
    ...
    return {
        ...
        "summary": {
            ...
            "leaderboard": {
                "score": rescue_ranking["score"] if rescue_ranking else None,
                "rank_scope": "rescue-platform",
                "rank": rescue_ranking["rank"] if rescue_ranking else None,
                "status": "scoring_active",
                "note": "Score and rank are supplied by canonical Platform Ranking Aggregation."
            }
        },
        "leaderboard": {
            "score": rescue_ranking["score"] if rescue_ranking else None,
            "rank": rescue_ranking["rank"] if rescue_ranking else None,
            "components": rescue_ranking["components"] if rescue_ranking else {},
            "formula": {
                "authority": "canonical-platform-ranking-aggregation"
            }
        },
...
def account_partner_dashboard(
    ...
    partner_rankings = ranking_context["partners"]

    primary_partner_ranking = (
        partner_rankings[0]
        if partner_rankings
        else None
    )
    ...
    return {
        ...
        "summary": {
            ...
            "leaderboard": {
                "score": primary_partner_ranking["score"] if primary_partner_ranking else None,
                "rank_scope": "partner-platform",
                "rank": primary_partner_ranking["rank"] if primary_partner_ranking else None,
                "status": "scoring_active",
                "note": "Score and rank are supplied by canonical Platform Ranking Aggregation."
            }
        },
        "leaderboard": {
            "score": primary_partner_ranking["score"] if primary_partner_ranking else None,
            "rank": primary_partner_ranking["rank"] if primary_partner_ranking else None,
            "components": primary_partner_ranking["components"] if primary_partner_ranking else {},
            "records": partner_rankings,
            "formula": {
                "authority": "canonical-platform-ranking-aggregation"
            }
        },
...
def account_network_pulse(
    ...
    pulse_counts = build_network_pulse_counts(db)
    ...
    return {
        ...
        "network": {
            "members": pulse_counts["network"]["members"],
            "organizations": pulse_counts["network"]["organizations"],
            "creators": pulse_counts["network"]["creators"],
            "rescues": pulse_counts["network"]["rescues"],
            "rescue_organizations": pulse_counts["network"]["rescue_organizations"],
            "affiliates": pulse_counts["network"]["affiliates"],
        },
        "activity": {
            "memorials": pulse_counts["activity"]["memorials"],
            "contributions": pulse_counts["activity"]["contributions"],
            "media_assets": pulse_counts["activity"]["media_assets"],
            "active_campaigns": pulse_counts["activity"]["active_opportunities"],
        },
        "featured": {
            "creator": None,
            "rescue": None,
        ...
        "notes": [
            ...
            "Canonical Pulse counts are supplied by the locked Network Pulse Count Aggregation Seam.",
            "Leaderboard is a separate network feature."
        ]
    }
