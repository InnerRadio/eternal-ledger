...
def account_platform_leaderboard(
    ...
    for platform_user in users:
        ...
        user_memberships = memberships_by_user_id.get(platform_user.id, [])
        ...
        active_orgs = [
            organizations_by_id.get(membership.organization_id)
            for membership in user_memberships
            if organizations_by_id.get(membership.organization_id)
        ]
        ...
        creator_score = (
            ...
            + (len(user_memorials) * 5)
            + (len(user_contributions) * 3)
            + (len(user_media) * 2)
            + (len(active_orgs) * 20)
        ...
        creator_rankings.append({
            ...
            "role": role,
            "affiliate_id": platform_user.affiliate_id,
            "referral_code": platform_user.referral_code,
            "score": creator_score,
            "components": {
                "clicks": len(user_clicks),
                "conversions": len(user_conversions),
                "campaign_enrollments": len(user_enrollments),
                "memorials": len(user_memorials),
                "contributions": len(user_contributions),
                "media_assets": len(user_media),
                "organizations": len(active_orgs),
                "commission_points": int(user_commission_cents / 100),
            }
        })
        ...
        rescue_rankings.append({
            ...
            "score": rescue_score,
            "components": {
                "rescue_organizations": len(rescue_orgs),
                "memorials": len(user_memorials),
                "contributions": len(user_contributions),
                "media_assets": len(user_media),
                "campaign_enrollments": len(user_enrollments),
    ...
    for organization in organizations:
        ...
        partner_rankings.append({
            ...
            "score": partner_score,
            "components": {
                "campaigns": len(org_campaigns),
                "campaign_enrollments": len(org_enrollments),
                "conversions": len(org_conversions),
                "clicks": len(org_clicks),
                "creator_relationships": len(creator_relationships),
                "rescue_relationships": len(rescue_relationships),
                "commission_points": int(org_commission_cents / 100),
            }
        })
    ...
    creator_rankings = ranked(creator_rankings)
    rescue_rankings = ranked(rescue_rankings)
    affiliate_rankings = ranked(affiliate_rankings)
    partner_rankings = ranked(partner_rankings)
    ...
    return {
        ...
        "leaderboards": {
            "creators": creator_rankings[:25],
            "rescues": rescue_rankings[:25],
            "partners": partner_rankings[:25],
            "affiliates": affiliate_rankings[:25],
        },
        "notes": [
...
def account_partner_campaigns(
    ...
    query = db.query(PartnerCampaign).filter(
        PartnerCampaign.project == project
    )

    if partner_organization_id:
        query = query.filter(
            PartnerCampaign.partner_organization_id == partner_organization_id
        )
    ...
    if not has_admin_access:
        organization_ids = [
            membership.organization_id
            for membership in db.query(OrganizationMember).filter(
                OrganizationMember.user_id == user.id,
                OrganizationMember.status == "active"
            ).all()
        ]
        ...
        query = query.filter(
            PartnerCampaign.partner_organization_id.in_(organization_ids)
        )
    ...
    return {
        ...
        "count": len(campaigns),
        "records": [
            serialize_partner_campaign(campaign, db)
            for campaign in campaigns
        ]
    }
...
def account_partner_campaign_detail(
    ...
    campaign = db.query(PartnerCampaign).filter(
    ...
    if not has_admin_access:
        membership = db.query(OrganizationMember).filter(
            OrganizationMember.user_id == user.id,
            OrganizationMember.organization_id == campaign.partner_organization_id,
            OrganizationMember.status == "active"
        ).first()
    ...
    return {
        ...
        "record": serialize_partner_campaign(campaign, db)
    }
...
def account_upsert_partner_campaign(
    ...
    if not has_admin_access:
        membership = db.query(OrganizationMember).filter(
            OrganizationMember.user_id == user.id,
            OrganizationMember.organization_id == partner_organization_id,
            OrganizationMember.status == "active"
        ).first()
        ...
        if not membership:
            return {
                "module": "Partner Campaigns",
                "status": "error",
                "message": "You do not have access to create or update campaigns for this organization."
            }
    ...
    if not campaign:
        ...
        campaign.start_date = parse_dt(start_date)
        campaign.end_date = parse_dt(end_date)
